...
from .MultiScaleFormer import build_multiscaleformer
...
class MultiScaleDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        p5 = self.input_proj5(p4)
        p5_temp = nested_tensor_from_tensor_list(p5)
        m5 = p5_temp.mask
        p5_pos = self.backbone[1](p5_temp)
        pos.pop(0)
        pos += p5_pos
...
def build_MultiScaleDETR(args, data_loader):
    ...
    multiscaleformer = build_multiscaleformer(args)

    model = MultiScaleDETR(
        backbone,
        multiscaleformer,
