...
def make_coco_transforms(image_set):
    ...
    if image_set == 'train':
        ...
        return T.Compose([
            T.RandomHorizontalFlip(),
            T.RandomResize([800]),  # 将RandomResize的尺度参数设置为[800]
            T.RandomSizeCrop(800, 1333),  # 将RandomSizeCrop的尺寸范围设置为800 * 1333
            normalize,
        ])
...
def build_powerline(image_set, args):
    root = Path(args.coco_path)
    assert root.exists(), f'provided COCO path {root} does not exist'
    PATHS = {
        "train": (root / "train", root / "annotations" / 'train.json'),
        "val": (root / "train", root / "annotations" / 'val.json'),
    }
